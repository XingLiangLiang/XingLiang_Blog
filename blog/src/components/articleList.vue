      ...
      <div v-else class="layer">
        ...
        <aside>
          <div class="author-inner">
            <img class="author-img" src="../../static/images/avatar.jpg" alt="">
            <h3>Jake</h3>
            <p>Android/Go/Vue</p>
            <br>
            <p>了解更多..</p>
            <ul>
              <li>
                <a href="https://github.com/wuqingjian" target="_blank">
  ...
  export default {
    ...
    methods: {
      requestData(currentPage) {
        ...
        let filter = 'created';
        let sort = 'updated';
        this.$axios.get(`${url}?access_token=${this.$store.state.githubToken[0]}${this.$store.state.githubToken[1]}&&labels=已审核&&filter=${filter}&&sort=${sort}`)
          .then((res) => {
          if (res.status === 200) {
        ...
        this.$axios.get(`${url}?access_token=${this.$store.state.githubToken[0]}${this.$store.state.githubToken[1]}&&labels=已审核&&page=${currentPage}&&per_page=${per_page}&&filter=${filter}&&sort=${sort}`)
          .then((res) => {
          if (res.status === 200) {
            this.list = res.data;
            this.loading = false;
